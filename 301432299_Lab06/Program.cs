...
namespace _301432299_Lab06
{
    ...
    {
        //To create a collection to work with in global scope
        ...
        static void Main(string[] args)
        {
            ...
            //1. Display all elements in the collection 'elements':
            Console.WriteLine("1. Display all items in the collection 'elements':");
            Console.WriteLine("");
            DisplayItems();
            Console.WriteLine("");
            ...
            //2. Seriliaze the first item using json format and save to a suitable file:
            Console.WriteLine("2. First item has been serialized and saved to a file.");
            SaveAtom("FirstAtom.json", elements);
            Console.WriteLine("");

            NextQuestion();

            //3. Read the above file and display the item:
            Console.WriteLine("3. Above file was read, de-serialized, and is displayed below:");
            ReadAtom("FirstAtom.json");

            NextQuestion();

            //4. Serialize the entire collection and save to a suitable file:
            Console.WriteLine("4. Entire collection has been serialized and saved to a file.");
            SaveAllAtoms("AllAtoms.json", elements);
            Console.WriteLine("");

            NextQuestion();

            //5. Read the above file and display all of the items:
            Console.WriteLine("5. The above file was read, de-serialized, and is displayed below:");
            Console.WriteLine("");
            List<Atom> allAtoms = ReadAllAtoms("AllAtoms.json");

            foreach (Atom element in allAtoms)
            {
                //Console.WriteLine(element); <- could display like this but want to make it more readable:
                Console.WriteLine("Atom Properties:");
                Console.WriteLine($"Element Name:{element.Name}.");
                Console.WriteLine($"Number of Protons: {element.Proton}.");
                Console.WriteLine($"Number of Neutrons: {element.Neutron}.");
        ...
        //1. Method to display all elements in the collection 'elements':
        ...
        //2. Method to seriliaze the first item using json format and save to a suitable file
        public static void SaveAtom(string fileName, List<Atom> elements)
        {
            //write to file:
            StreamWriter writer = new StreamWriter(fileName);

            //need to save first element only
            Atom first = elements.First();
            //serialize this element:
            string elementJson = JsonSerializer.Serialize(first);

            //write to the file
            writer.WriteLine(elementJson);
        ...
        //3. Read the above file and display the item:
        public static void ReadAtom(string fileName)
        {
            StreamReader reader = new StreamReader(fileName);
            string first = reader.ReadLine(); //will only read first line (the only line in our file)

            Atom element = JsonSerializer.Deserialize<Atom>(first); //specifying which class to deserialize to 
            //Console.WriteLine(element); <- could display like this but want to make it more readable:
            Console.WriteLine("Atom Properties:");
            Console.WriteLine($"Element Name:{element.Name}.");
            Console.WriteLine($"Number of Protons: {element.Proton}.");
        ...
        //4. Serialize the entire collection and save to a suitable file:
        public static void SaveAllAtoms(string fileName, List<Atom> elements)
        {
            // write to file:
            StreamWriter writer = new StreamWriter(fileName);

            //then use a loop to save each element to the file:
        ...
        //5. Read the above file and display all of the items:
        public static List<Atom> ReadAllAtoms(string fileName)
        {
            List<Atom> allAtoms = new List<Atom>(); //declare a new list 

            StreamReader reader = new StreamReader(fileName);
            string line = reader.ReadLine();

            while (line != null) //condition = while there is a line to read, continue reading
            {
                Atom element = JsonSerializer.Deserialize<Atom>(line); //specifying which class to deserialize to 
                allAtoms.Add(element); // add items to the list 
        ...
        //6. BONUS METHOD for readability in the console:
        public static void NextQuestion()
        {
            Console.WriteLine("Please press enter to continue to the next question.");
            Console.ReadLine();
        }
